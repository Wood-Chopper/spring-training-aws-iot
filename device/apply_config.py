...
from awsiot import iotjobs, iotshadow, mqtt5_client_builder
...
def on_publish_update_shadow(future):
    ...
    try:
        future.result()
        print("Update shadow request published.")
    except Exception as e:
        print("Failed to publish update shadow request.")
        raise e

def update_shadow(job_document):
    ...
    future = shadow_client.publish_update_named_shadow(request, mqtt5.QoS.AT_LEAST_ONCE)
...
def job_thread_fn(job_id, job_document):
    try:
        ...
        print("Publishing request to update job status to FAILED...")
        request = iotjobs.UpdateJobExecutionRequest(
            thing_name=jobs_thing_name,
            job_id=job_id,
